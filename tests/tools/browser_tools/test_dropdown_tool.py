...
from src.ii_agent.browser.browser import Browser, BrowserPage, BrowserState, InteractiveElement # For type hinting and spec
...
class CommonDropdownToolTests(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_state = MagicMock(spec=BrowserState)
...
class TestBrowserGetSelectOptionsTool(CommonDropdownToolTests):
    ...
    def test_run_get_options_success(self):
        ...
        mock_select_element = InteractiveElement(
            index=element_index, tag_name="select", browser_agent_id=element_agent_id,
            aria_label="Test Select", role="combobox", text_content=None, html_attributes={}
        )
    ...
    def test_run_get_options_element_not_found(self):
        ...
        self.assertEqual(result.output_for_llm, "No element found with index 99")
        self.mock_page.evaluate.assert_not_called()

    def test_run_get_options_element_not_select(self):
        element_index = 2
        mock_button_element = InteractiveElement(index=element_index, tag_name="button", browser_agent_id="btn-id", aria_label="A button", role="button", text_content="Click", html_attributes={})
        self.mock_state.interactive_elements = {element_index: mock_button_element}

        result = self._run_tool_async(self.tool, {"index": element_index})
        self.assertEqual(result.output_for_llm, f"Element {element_index} is not a select element, it's a button")
        self.mock_page.evaluate.assert_not_called()
...
class TestBrowserSelectDropdownOptionTool(CommonDropdownToolTests):
    ...
    def test_run_select_option_success(self):
        ...
        mock_select_element = InteractiveElement(index=element_index, tag_name="select", browser_agent_id=element_agent_id, aria_label="Another Select", role="combobox", text_content=None, html_attributes={})
        self.mock_state.interactive_elements = {element_index: mock_select_element}
    ...
    def test_run_select_option_js_returns_failure_option_not_found(self):
        ...
        mock_select_element = InteractiveElement(index=element_index, tag_name="select", browser_agent_id=element_agent_id, aria_label="Test Select", role="combobox", text_content=None, html_attributes={})
        ...
        self.assertEqual(result.output_for_llm, expected_llm_output)
    ...
    def test_run_select_option_element_not_found_in_state(self):
        ...
        self.assertEqual(result.output_for_llm, "No element found with index 100")

    def test_run_select_option_element_not_select_tag(self):
        element_index = 5
        mock_input_element = InteractiveElement(index=element_index, tag_name="input", browser_agent_id="input-id", aria_label="An input", role="textbox", text_content=None, html_attributes={})
        self.mock_state.interactive_elements = {element_index: mock_input_element}

        result = self._run_tool_async(self.tool, {"index": element_index, "option": "Any"})
        self.assertEqual(result.output_for_llm, f"Element {element_index} is not a select element, it's a input")
