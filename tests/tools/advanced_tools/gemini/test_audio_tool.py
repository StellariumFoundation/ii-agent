...
class CommonGeminiAudioToolTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Patch the genai.GenerativeModel constructor used in GeminiTool base class
        self.genai_client_patcher = patch('google.generativeai.GenerativeModel')
        self.MockGenaiClientConstructor = self.genai_client_patcher.start()

        self.mock_gemini_client_instance = MagicMock(spec=genai.GenerativeModel) # spec is now GenerativeModel
        self.MockGenaiClientConstructor.return_value = self.mock_gemini_client_instance

        # This is what will be called by the tools: self.client.generate_content (assuming client is GenerativeModel instance)
